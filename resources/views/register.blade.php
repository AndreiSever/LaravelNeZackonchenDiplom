...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">{{ __('Регистрация для студентов') }}</div>

                <div class="card-body">
                    <form method="POST" action="{{ route('registerStudentShow') }}">
                        @csrf
                        ...
                        <div class="form-group row">
                            <label for="controlSelect" class="col-md-4 col-form-label text-md-right">{{ __('Номер группы') }}</label>
                            <div class="col-md-6">
                            <select class="form-control" id="controlSelect" name="number_group">
                            @if($group)
                                @foreach($group as $groups)
                                <option>{{$groups['number_group']}}</option>
                                @endforeach
                            </select>
                            @else
                            <option>-</option>
                            ...
                            <!-- @if ($errors->has('number_group'))
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $errors->first('number_group') }}</strong>
                                </span>
                            @endif -->
                            </div> 
                        </div>
